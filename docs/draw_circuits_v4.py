"""Generate visual diagrams of the 9+3 synthetic eval circuits — v4
Rotated component polygons, pin dots, wire endpoints, net junctions.
"""
...
def rotated_rect(cx, cy, w, h, angle_deg):
    """Return 4 corners of a rectangle centered at (cx, cy) rotated by angle_deg."""
...
def draw_component_polygon(draw, cx, cy, w, h, angle, ctype, label, value):
    """Draw component as a rotated polygon with fill and border."""
    fill, border = TYPE_COL.get(ctype, ("#333", "#666"))
    corners = rotated_rect(cx, cy, w, h, angle)
...
def draw_circuit(draw, spec, cell_x, cell_y):
    ...
    for i, c in enumerate(spec.comps):
        ...
        bw = raw_w * scale
        bh = raw_h * scale
        draw_component_polygon(draw, cx, cy, max(bw, 32), max(bh, 24), angle, c.type,
