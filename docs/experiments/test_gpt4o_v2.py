#!/usr/bin/env python3
"""GPT-4o-mini with explicit provider routing."""
...
def pipe(gray,k=0.3,w=51,cks=3,cma=20,da=10,dd=18,ma_=10,mg_=30,ml=10):
    ...
    while ch:
        ch=False;i=0;rng=list(range(len(r)))
        while i<len(r):
            j=i+1
            while j<len(r):
                p1,p2=r[i];q1,q2=r[j]
...
for k in [0.25,0.30,0.35]:
    ...
    pl={"model":"openai/gpt-4o-mini",
        "provider":{"order":["OpenAI"],"allow_fallbacks":False},
        "messages":[{"role":"user","content":[
            {"type":"text","text":"Rate this circuit trace binarization 1-5 (1=no traces, 2=faint, 3=some visible, 4=most visible, 5=crisp complete). Number only."},
            {"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}
        ]}],"max_tokens":5}
    r=requests.post("https://openrouter.ai/api/v1/chat/completions",headers=hdrs,json=pl,timeout=15)
    
    if r.status_code==200:
        g=r.json()["choices"][0]["message"]["content"].strip()[:10]
        print(f"k={k:.2f}  F1={f1:.4f}  trace={trace:.3f}%  GPT4o={g}")
    else:
        e=r.json().get("error",{}).get("message","")[:100]
        print(f"k={k:.2f}  F1={f1:.4f}  trace={trace:.3f}%  ERROR: {e}")
