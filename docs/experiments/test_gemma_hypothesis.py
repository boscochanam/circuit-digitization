#!/usr/bin/env python3
"""Quick gemma-validated k sweep on worst images. Proves the threshold-ceiling hypothesis."""
import os, base64, math, requests
...
def pipe(gray,k=0.3,w=51,cks=3,cma=20,da=10,dd=18,ma_=10,mg_=30,ml=10):
    ...
    bw=(gray>mean*(1+k*(std/128-1))).astype(np.uint8)*255
...
# Worst 4 images with no HDC
bad = ["C101_D1_P1","C10_D2_P3","C103_D2_P4","C100_D1_P1"]

print("="*70)
print("Can manual k-adjustment make these near-perfect?")
print("="*70)
...
for stem in bad:
    ...
    best_actual={"f1":0}
    gemma_best={"f1":0,"k":0.3,"rating":0}
    
    for k in [0.10,0.15,0.20,0.25,0.30,0.35,0.40,0.45]:
        ...
        if f1>best_actual["f1"]:
            best_actual={"f1":f1,"k":k,"tp":t,"fp":f+rd,"fn":fn2,"n":len(lines)}
        
        # Gemma rating
        trace=(bw>0).mean()*100
        cv2.imwrite(f"/tmp/g_{stem}_k{k:.2f}.jpg",bw)
        with open(f"/tmp/g_{stem}_k{k:.2f}.jpg","rb") as fh:
            b64=base64.b64encode(fh.read()).decode()
        hdrs={"Authorization":f"Bearer {AK}","Content-Type":"application/json"}
        pl={"model":"deepseek/deepseek-v4-pro",
            "messages":[{"role":"user","content":[
                {"type":"text","text":"Rate this binarization 1-5 (1=no traces, 5=perfect). Number only."},
                {"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}]}],
            "max_tokens":5}
        try:
            r=requests.post("https://openrouter.ai/api/v1/chat/completions",
                headers=hdrs,json=pl,timeout=15)
            g=int(r.json()["choices"][0]["message"]["content"].strip()[0]) if r.status_code==200 else 0
        except:
            g=0
        
        if g>gemma_best["rating"] or (g==gemma_best["rating"] and f1>gemma_best["f1"]):
            gemma_best={"f1":f1,"k":k,"tp":t,"fp":f+rd,"fn":fn2,"n":len(lines),"rating":g,"trace":trace}
        
        if f1>0.1 or g>=3:
            m_g="← gemma" if g>=3 else ""
            print(f"  k={k:.2f} F1={f1:.4f} tp={t} fp={f+rd} fn={fn2} lines={len(lines)} trace={trace:.3f}% gemma={g} {m_g}")
    
    print(f"  Actual best: k={best_actual['k']:.2f} F1={best_actual['f1']:.4f} tp={best_actual['tp']} fp={best_actual['fp']} fn={best_actual['fn']}")
    print(f"  Gemma best:  k={gemma_best['k']:.2f} F1={gemma_best['f1']:.4f} rating={gemma_best['rating']}/5 tp={gemma_best['tp']} fp={gemma_best['fp']} fn={gemma_best['fn']}")
