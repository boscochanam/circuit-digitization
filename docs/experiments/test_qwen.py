#!/usr/bin/env python3
"""Qwen 2.5 VL 72B vision rating vs actual F1 on failing images."""
...
for stem in bad:
    ...
    best_f1 = 0
    for k in [0.10,0.15,0.20,0.25,0.30,0.35,0.40]:
        ...
        cv2.imwrite(f"/tmp/q_{stem}_k{k:.2f}.jpg",bw)
        with open(f"/tmp/q_{stem}_k{k:.2f}.jpg","rb") as fh:
            b64=base64.b64encode(fh.read()).decode()
        
        r=requests.post("https://openrouter.ai/api/v1/chat/completions",
            headers={"Authorization":f"Bearer {AK}"},
            json={"model":"qwen/qwen-2.5-vl-72b-instruct",
                  "messages":[{"role":"user","content":[
                      {"type":"text","text":"This is a circuit trace binarization result. White=traces, Black=background. Rate 1-5: 1=empty/no traces, 2=faint/barely visible, 3=some traces but broken, 4=most traces visible and continuous, 5=all traces crisp and complete. Reply with ONLY the number."},
                      {"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}
                  ]}],"max_tokens":5},timeout=20)
        
        if r.status_code==200:
            g=r.json()["choices"][0]["message"]["content"].strip()[:10]
        else:
            g=f"ERR"
        
        marker = " ◀ HIGHEST F1" if f1 == best_f1 else ""
        print(f"  k={k:.2f}  F1={f1:.4f}  trace={trace:.4f}%  Qwen={g}{marker}")
