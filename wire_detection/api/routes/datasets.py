...
def get_thumb(idx: int = 0, ds: str = Query("gt_labels")):
    ...
    try:
        path = str(images[idx])
        img = _server.cache.load_image(path, resize=300)
...
def list_datasets():
    info = {}
    for key in _server.registry.list_datasets():
        cfg = _server.registry.get(key)
        images = _server.registry.list_images(key)
        info[key] = {
