...
def main() -> int:
    ...
    # summary
    print(f"\nReal-image join connectivity F1 ({len(per_image)} images, vs net-GT)")
    print(f"{'strategy':<16}{'meanF1':>8}")
    print("-" * 24)
    rows = sorted(((s, sum(v) / len(v) if v else 0.0) for s, v in per_strategy.items()),
                  key=lambda x: -x[1])
    for s, mf in rows:
        print(f"{s:<16}{mf:>8.3f}")

    Path(args.out).parent.mkdir(parents=True, exist_ok=True)
    json.dump({"per_image": per_image,
               "mean_f1": {s: (sum(v) / len(v) if v else 0.0) for s, v in per_strategy.items()}},
