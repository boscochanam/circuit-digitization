...
\markboth
{Chanam \lowercase{\textit{et al.}}: From Hand-Drawn Schematics to SPICE Netlists}
{Chanam \lowercase{\textit{et al.}}: From Hand-Drawn Schematics to SPICE Netlists}
...
\begin{abstract}
Automated digitization of scanned analog circuit diagrams into simulation-ready SPICE netlists remains a challenging problem in document understanding. We present a complete deterministic pipeline that converts a scanned hand-drawn schematic into a SPICE netlist through component detection, an occlusion-first wire extractor, and a graph-based join that resolves electrical connectivity. Within this pipeline the dominant failure mode is the \emph{joining} step: determining which extracted wire segments connect to which component pins and to each other; existing approaches use radius-based proximity grabbing, which over-merges components onto shared nets under the noisy endpoint estimates produced by real detectors. We contribute (1)~an occlusion-first wire extractor that masks detected components before binarization and estimates per-segment endpoints, yielding a line-segment-with-endpoints representation (rather than tracing nets directly) that reaches wire-detection F1~=~0.976 on 134 scanned images and is what enables graph-based joining; (2)~an endpoint-graph join model that represents both wire endpoints and component pins as graph nodes with five edge types capturing wire-body, endpoint-endpoint, endpoint-pin, endpoint-to-wire-body (T-junction), and pin-to-wire-body (rail-tap) connectivity; (3)~a degree-budget completion algorithm that uses min-cost b-matching to recover dropped connections while structurally bounding over-merge through a per-pin edge budget and self-loop guards; (4)~a synthetic evaluation framework with controlled error injection across five severity levels, for robustness analysis independent of ground-truth wire labels; and (5)~to our knowledge, the first human-verified net-level connectivity benchmark for hand-drawn circuits (31 images from CGHD-1152), on which we compare join algorithms under the same metric, a comparison no prior image-to-netlist work provides. On the human-verified net-level benchmark, the best join (degree-budget completion over a high-precision scale-relative endpoint-graph base) reaches connectivity micro-F1~=~0.890, versus 0.83 for the prior completion default, 0.67 for the radius-based production join, and 0.62 for the connected-component net-tracing recipe used by recent systems, all evaluated on identical detected wires; it also beats a Hough-plus-proximity line tracer (0.81). Running the join on perfect wire labels does not raise micro-F1, showing that connectivity, not wire detection, is the remaining bottleneck within the pipeline. A first-party experiment shows a strong vision-language model reaches 0.923 on the same images, statistically indistinguishable from our join (paired 95\% confidence interval includes zero) but at orders-of-magnitude higher cost, with no structural guarantee and non-simulatable output, supporting an explicit geometric model for deployable digitization.
\end{abstract}

\begin{IEEEkeywords}
hand-drawn circuits, circuit digitization, schematic recognition, document understanding, image-to-netlist, wire extraction, wire joining, graph-based connectivity, netlist extraction, SPICE, connectivity benchmark
\end{IEEEkeywords}
...
\texttt{scale\_completion} (ours) & \textbf{0.890} & 0.919 & 0.864 & \textbf{0.901} \\
degree\_budget (prior default) & 0.829 & 0.789 & 0.874 & 0.853 \\
graph\_scale & 0.816 & 0.929 & 0.727 & 0.838 \\
graph\_rescue & 0.787 & 0.811 & 0.764 & 0.813 \\
...
\texttt{scale\_completion}, perfect wires & 0.890 & 0.913 & 0.868 & 0.916 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\textbf{Detection is not the bottleneck.} Running the best join on the dataset's perfect wire labels rather than detected wires leaves micro-F1 unchanged at 0.890 (it raises macro-F1 only from 0.901 to 0.916; Table~\ref{tab:real_join}), so the wire detector accounts for essentially zero pair-weighted F1. The residual error is intrinsic join ambiguity on hand-drawn geometry, not wire detection, consistent with the 0.976 wire-detection F1 reported above (Table~\ref{tab:wire_benchmark}). The join degrades only gently with circuit size (per-image F1 falls from 0.94 on circuits with $\leq$6 components to 0.86 above that, weak correlation $r=-0.19$) and remains high-precision throughout, in contrast to the VLM, whose precision collapses on the densest circuits (Section~\ref{sec:vlm_results}).
