...
\item \textbf{Candidate edges:} For each floating pin $p$, generate candidate edges to all pins $q$ in other components within reach $r = \rho \cdot s$, where $\rho$ is the reach factor (we use $\rho = 4$ for the deployed scale-relative join; Section~\ref{sec:real_eval} shows a broad F1 plateau for $\rho \in [3,5]$). Edge weight is Euclidean distance $\|p - q\|$.
...
\textbf{Simulation accuracy:} Percentage of random seeds where every source current in the predicted SPICE netlist matches the clean oracle within 5\%.
...
Rescue graph (base) & 1.00 & 1.00 & 1.00 & 0.94 & 0.89 \\
...
\midrule
Parallel R--R & 3 & 0.97 & 1.00 & 25\% \\
R--R divider & 3 & 0.91 & 0.94 & 50\% \\
RL series & 3 & 0.91 & 1.00 & 62\% \\
Diode--R loop & 3 & 0.91 & 1.00 & 50\% \\
Dual R loops & 4 & 0.96 & 1.00 & 12\% \\
4-component loop & 4 & 0.87 & 0.97 & 44\% \\
Wheatstone bridge & 6 & 0.76 & 0.85 & 12\% \\
6-component ring & 6 & 0.79 & 0.97 & 12\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Precision remains high ($\geq$~0.85) across all circuits even at L4: errors cause \emph{fragmentation} (dropped connections) rather than \emph{shorts} (false merges). The 6-component ring is the hardest topology: its cyclic structure means a single dropped connection breaks the loop, reducing simulation accuracy to 12\%.
