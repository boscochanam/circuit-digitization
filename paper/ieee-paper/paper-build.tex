...
\begin{table}[htbp]
\caption{Class Merges: 61 CGHD Classes Reduced to 16. The Eight Merged Targets Are Shown; Seven Classes Pass Through Unchanged and 30 Rare Classes Fold Into \texttt{other}.}
\label{tab:class_merge}
\begin{center}
\footnotesize
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{0.52\columnwidth}@{}}
\toprule
\textbf{Merged class} & \textbf{CGHD source classes} \\
\midrule
\texttt{resistor} & resistor, .adjustable, .photo \\
\texttt{capacitor} & capacitor.unpolarized, .polarized, .adjustable \\
\texttt{diode} & diode, .zener, .light\_emitting, .thyrector \\
\texttt{transistor} & transistor.bjt, .fet, .photo \\
\texttt{inductor} & inductor, .ferrite, .coupled \\
\texttt{voltage\_source} & voltage.battery, .dc, .ac \\
\texttt{integrated\_circuit} & integrated\_circuit, .ne555, .voltage\_regulator \\
\texttt{operational\_amplifier} & operational\_amplifier, .schmitt\_trigger \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
