...
|---|---|---|---|
| R1-1 single collection, 31 images | Only 31 images from a single collection, with no proof the method works on other drawing styles or at larger scale. | Results labeled as within-collection only, with drafter groups presented descriptively (16 direct and 15 inferred assignments disclosed, per-drafter table included); no broad claims and no held-out-drafter experiment; future expansion described as future work. | Owner read-through |
